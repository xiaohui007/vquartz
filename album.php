...
$page = GetGP('page');
$count =$_SGLOBAL['db']->get_value("SELECT count(*) FROM ".tname('active')." A ".$tablesql." WHERE A.display='1' ".$wheresql);
...
$list = array();
if($count>0){
  $query = $_SGLOBAL['db']->query("select A.* from ".tname('active')." A ".$tablesql." where A.display='1' $wheresql ORDER BY $orderby LIMIT $start,$perpage ");
...
//读取分类
$query = $_SGLOBAL['db']->query("select * from ".tname('xilie')."  where display='1' AND type='2' ORDER BY sort ASC ");
while($value = $_SGLOBAL['db']->fetch_array($query)){      
  $xilielist[] = $value;
}

$title="相册"; $index_nav="3"; include 'inc/header.php'; 
?>
<style type="text/css">
		...
		<div class="inner">
			<div class="menu">
				...
				<ul>
					...
					?>
					<li><a <?php if($val['id']==$xilie){echo 'class="active"';} ?> href="album.php?xilie=<?php echo $val['id']; ?>"><?=$val['name']?></a></li>
					<?php
					}
					?>
				</ul>
			...
			<?php if($list){ ?>
				...
				<?php foreach ($list as $key => $value) {?>
					<li>
						<a class="m_graphic" href="album_detail.php?id=<?php echo $value['id'];?>">
							<img title="<?php echo $value['title']; ?>" src="<?php echo $value['img']; ?>">
							<i></i>
						</a>
					</li>
				<?php } ?>
