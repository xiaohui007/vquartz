...
if (submitcheck('listsubmit')) {
	...
	if($_POST['uids'] && is_array($_POST['uids']) && $_POST['optype']) {
		$url = "news.php?perpage=$_GET[perpage]&page=$_GET[page]";
		switch ($_POST['optype']) {
			case '3':
				//删除
				$_SGLOBAL['db']->query("DELETE FROM ".tname('news')." WHERE id IN (".simplode($_POST['uids']).")");
				break;
		}
	}
	cpmessage('操作成功', $url);
	...
	$csql = "SELECT COUNT(*) FROM ".tname('news')." b WHERE $wheresql";
	...
	$qsql = "SELECT $selectsql FROM ".tname('news')." b WHERE $wheresql $ordersql LIMIT $start,$perpage";
...
if($list){
?>
	<form method="post" action="news.php?perpage=<?=$perpage?>&page=<?=$page?>" onsubmit="return show_detail();">
	<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
	<div class="bdrcontent">
		<div class="topactions"><a href="news.php">浏览</a> | <a href="news_mamage.php?action=add">添加</a></div>
		...
		<tr>
			...
			<td>
				<img src="<?=$value['src']?>" height="100"  />
			</td>
			<td>
				<?=$value['date']?>
			</td>
			<td align="center">
				<a href="news_mamage.php?id=<?=$value['id']?>&action=edit"> 管理</a>
			</td>
		</tr>
		<?php
		}
